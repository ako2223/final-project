...
function RegistrationForm() {
  ...
  return (
            <form onSubmit={handleSubmit(onSubmit)} className={style.form} noValidate>
                <div className={style.input}><input className={errors.name && style.error} type="text" placeholder="Name" {...register("login", {pattern:  {value: /^[a-zA-Z0-9]{2,15}$/
    , message: "Username must be 2-15 characters long, contain only letters and numbers"}, required: 'Required field'})} />
                {errors.login &&<span className={style.error}>{errors.login.message}</span>}</div>
    ...
    , message: "Invalid email format"}, required: 'Required field'})}/>
                ...
                <div className={style.input}><input className={errors.password && style.error} type="password" placeholder="Password" {...register("password", {pattern:  {value: /^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?!.*\s)[a-zA-Z0-9]*.{8,20}$/
